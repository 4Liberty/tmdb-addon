...
function parseImdbLink(
  ...
  return {
    name:
      showAgeRatingWithImdbRating && ageRating
        ? `${ageRating}\u2003\u2003${vote_average}`
        : vote_average,
    category: "imdb",
...
async function parseMediaImage(
  type,
  id,
  imagePath,
  language,
  ...
  if (rpdbkey && rpdbMediaTypes) {
    const isMediaTypeEnabled =
      rpdbMediaTypes[mediaType] === true ||
      (mediaType === "poster" && rpdbMediaTypes.poster !== false);
